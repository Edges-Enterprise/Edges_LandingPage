...
8. **RESOLVED 2026-09-06.** `src/middleware/*` (6 files, all empty) has been deleted — product owner confirmed all middleware logic stays in `src/proxy.ts` (Next.js 16's routing-middleware replacement), no split was ever intended. Any future route-scoped access logic (e.g. the RBAC route enforcement in Task 1a.i.x) belongs inside `proxy.ts`, not a new file under a resurrected `src/middleware/`.
9. **RESOLVED 2026-09-06.** `src/lib/supabase/server.ts`'s `createServerClient` ran on the service-role key instead of the anon key (discovered while fixing item 5's naming issue) — every call through the 126-file-imported cookie-bound client had full admin privileges and bypassed Postgres RLS regardless of whose session cookie was attached. Product owner made the call to switch it to the anon key so RLS actually applies per `auth.uid()`. Before flipping the key, the 15 `global_` tables were audited against the switch: 6 had no working RLS (`global_reseller_settings` had RLS enabled with zero policies → deny-all; `global_reseller_applications` — which holds `bvn` and a plaintext `temp_password` column —, `global_reseller_stores`, `global_reseller_app_configs`, `global_store_visits`, `global_app_builds`, `global_email_logs` had no RLS enabled at all → open to any grantee via the client-visible anon key). Closed in `supabase/migrations/20260906010000_rbac_foundation_rls.sql` with owner-scoped policies mirroring the pattern already live on `global_customers`/`global_wallets`. Separately, 6 machine-to-machine call sites that have no user session to run under were switched from `createServerClient` to `createAdminClient` (service-role, intentional bypass) instead of relying on the old RLS-bypass side effect: the build/payment/korapay/xixapay/lizzysub webhooks and the `/api/v1` `X-API-Key` auth middleware. **Not done as part of this fix, flagged for a future pass:** the other ~120 `createServerClient` call sites were not individually audited for correctness against RLS now actually applying — only the ones with an obvious no-session pattern (webhooks, API-key auth) were found and fixed. `lizzysub`'s webhook also has no signature verification at all (separate pre-existing gap, unrelated to this fix, not addressed here). `triggerAppBuild.ts` and `getBuildContext.ts` each declare an unused `createAdminClient` instance that was never wired to anything — harmless but worth cleaning up whenever those files are next touched.
