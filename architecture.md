...
9. **`src/lib/supabase/server.ts`'s `createServerClient` runs on the service-role key, not the anon key — discovered 2026-09-06 while fixing the `NEXT_PUBLIC_SUPABASE_SERVICE_ROLE_KEY` naming issue (item 5 above).** `@supabase/ssr`'s cookie-based `createServerClient` pattern (used here for `getUser()`/`getSession()` and imported in **122 files** across the app) is normally paired with the anon key specifically so Postgres RLS applies per the authenticated user from their session cookie. Wiring it to the service-role key instead means every call through this client runs with full admin privileges and bypasses RLS entirely, regardless of which user's cookie session is attached. This was only renamed (per item 5's fix), not changed semantically — **whether this is intentional (the app relies on application-code authorization instead of RLS, which is a valid but different security model) or an oversight (RLS is assumed elsewhere but silently never enforced) is an open question requiring a direct product-owner/engineering decision before anyone changes which key this file uses.** Given the file's 122-file blast radius, this is now one of the highest-severity open findings in the whole codebase — on par with the missing test coverage and the unversioned schema.
