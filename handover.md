...
```
CURRENT POSITION: STEP 0 — 0.2 and 0.3 done. 0.1 (schema capture) BLOCKED,
  needs live Supabase CLI access this environment doesn't have.
NEXT ACTION: Either (a) someone with Supabase project access runs 0.1 first,
  or (b) product owner explicitly accepts proceeding without it — either
  way, log the decision below, then start Task 1a.i.x.
```
...
- [ ] **0.1 — Database schema capture. BLOCKED — needs the product owner/an engineer with live Supabase project access; the sandbox this documentation work runs in has no credentials for the real Supabase project.** Run `supabase db dump`/`db pull` (or `supabase db pull` for a migration-history-aware capture) against the live project and commit the result under `supabase/migrations/`. This is the prerequisite every other task implicitly depends on (`master-architecture.md` §4's entire data model is a *guess* reconciled from working code until this happens) — **whoever has Supabase CLI access should run this before Task 1 begins**, or explicitly decide to proceed without it and accept the risk, recorded in the Session Log below if so.
