...
```
CURRENT POSITION: STEP 0 done. Known Irregularity #9 (server.ts RLS bypass)
  and #8 (middleware/proxy split) both resolved. Task 1a.i.x not yet started —
  its actual roles/permissions schema and route-enforcement logic are still
  to be built.
NEXT ACTION: Read supabase/migrations/20260906000000_baseline_live_schema.sql
  ...
  "guess reconciled from working code" data model. Then build Task 1a.i.x's
  roles/permissions tables and wire route-scoped enforcement into
  src/proxy.ts (NOT a new src/middleware/ file — that folder is deleted for
  good, see Known Irregularity #8 in architecture.md).
```
...
- [ ] 1a.i.x — Build the RBAC model: `roles`/`permissions` tables, the full role list from `master-architecture.md` §1.1 (`SUPER_ADMIN`, `PLATFORM_ADMIN`, `COMPLIANCE_OFFICER`, `FINANCE_ADMIN`, `SUPPORT_AGENT`, `MERCHANT_OWNER`, `MERCHANT_STAFF`, `CUSTOMER`, `API_CLIENT`), no `roles`/`permissions` table exists yet in `schema.sql` — confirmed empty search. **`src/middleware/` no longer exists (deleted 2026-09-06, see Known Irregularity #8) — role-scoped route enforcement belongs in `src/proxy.ts`, not a resurrected middleware file.** The RLS foundation this depends on (session client actually respecting `auth.uid()`) is now fixed — see Known Irregularity #9, resolved.
...
**Known irregularities still unresolved (do not rediscover these — act on them where the task board above references them, or flag if you find they've changed):** the 53 RPC files under `supabase/rpc/**` are still empty (0 bytes) — Step 0.1's schema capture did not populate these, since `pg_dump --schema-only` was scoped to `public`/`api_users` tables/policies/functions actually present in the live DB, and these RPC files describe functions that may not exist there yet or exist under different names — needs a follow-up audit against the 63 real functions in the baseline migration, `wallet-withxixicopy.ts`/`WalletClient-withxixicopy.tsx` confirmed dead code (safe to delete once product owner confirms), the `build/` vs `publishing/` action-namespace duplication (Task 3b.i — note `completeBuild.ts` and `getBuildContext.ts` in the `build/` namespace are dead code, never called from anywhere but their own barrel export), the `generateIcon.ts`/`logo/generator.ts` duplicate pair (fold into whichever task touches app branding), `src/app/api/lizzysub/webhook/route.ts` has no signature verification at all (every other payment webhook in the repo verifies one) — needs a fix whenever that integration is next touched, `src/app/api/reseller/[countryCode]/webhooks/build/route.ts` also has no auth/signature check on an endpoint that can mark any build "completed" with arbitrary APK/AAB URLs — same category of gap, not yet fixed, ~120 of the 126 `createServerClient` call sites have not been individually audited for correctness now that RLS actually applies (only the 6 no-session machine-to-machine sites were found and fixed — see Known Irregularity #9 in architecture.md) — a session working in any dashboard/action area should sanity-check that its reads/writes still succeed under RLS rather than assume they do.

**Resolved this session (2026-09-06), do not re-flag:** migration stubs at `supabase/migrations_old_empty/` deleted per product-owner instruction (they guessed at table names that didn't match the real schema). `src/middleware/` (6 empty files) deleted per product-owner instruction — all routing middleware lives in `src/proxy.ts` (Next.js 16 pattern), confirmed via repo-wide grep that nothing imported those files. `src/lib/supabase/server.ts` now runs on the anon key instead of the service-role key (Known Irregularity #9 in architecture.md), paired with a new RLS migration (`supabase/migrations/20260906010000_rbac_foundation_rls.sql`) closing the gap this surfaced on 6 `global_` tables, and 6 webhook/API-key-auth call sites switched to `createAdminClient` since they have no user session to run under. DB naming convention confirmed against `schema.sql`: `global_` prefix = Layer 3 (active work, 15 tables), `reseller_` prefix = Layer 2 (complete, reference-only, 15 tables), unprefixed = Layer 1 legacy (~35 tables). No Supabase Edge Functions exist anywhere in this repo or the captured schema — `pg_dump` only captures Postgres-native functions (all 63 are `plpgsql`/`sql`), Edge Functions are separately-deployed Deno code and `supabase/functions/` doesn't exist here; the docs only propose one for a future maintenance-billing cron job.
...
- **2026-09-06 (later still)** — Product owner gave explicit direction on four items, none of which is Task 1a.i.x itself but all of which unblock it: (1) deleted the 13 empty migration stubs at `supabase/migrations_old_empty/`; (2) deleted `src/middleware/` (6 empty files, zero imports anywhere — confirmed via grep before deleting) since Next.js 16 routing middleware lives entirely in `src/proxy.ts`, resolving Known Irregularity #8; (3) fixed `src/lib/supabase/server.ts` to run on the anon key instead of the service-role key, resolving Known Irregularity #9 — this required auditing all 15 `global_` tables' RLS coverage first, which surfaced 6 with none (`global_reseller_settings` deny-all, `global_reseller_applications`/`global_reseller_stores`/`global_reseller_app_configs`/`global_store_visits`/`global_app_builds`/`global_email_logs` wide open), closed in a new migration `supabase/migrations/20260906010000_rbac_foundation_rls.sql` with owner-scoped policies matching the existing `global_customers`/`global_wallets` pattern, plus switching 6 no-session call sites (5 payment/build webhooks + the `/api/v1` API-key auth middleware) from `createServerClient` to `createAdminClient` since the RLS-respecting client has no session to run under in those cases; (4) confirmed the `global_`/`reseller_`/unprefixed table-naming convention against `schema.sql` (Layer 3 / Layer 2-reference-only / Layer 1 respectively) and confirmed no Supabase Edge Functions exist anywhere in this repo (`pg_dump` only captures Postgres-native functions; Edge Functions are separate Deno deployments and `supabase/functions/` doesn't exist here). **New findings surfaced and logged in Known Irregularities above, not fixed this session:** ~120 of 126 `createServerClient` call sites weren't individually re-audited against RLS now actually applying; `lizzysub` and the reseller build webhook have no request-auth/signature check at all; `completeBuild.ts`/`getBuildContext.ts` are dead code; two files have unused `createAdminClient` instances. `CURRENT POSITION` remains at Task 1a.i.x — its actual schema/logic still needs to be built next.
