...
export async function generateIconPng(
  ...
  // Every hardcoded pixel value below was tuned for a 1024px canvas.
  // SCALE lets the same drawing code run at any resolution (e.g. 192px
  // for cheap live previews) without redrawing the whole thing by hand.
  ...
  const cardSize = S(700);
  const cardX = cx - cardSize / 2;
  const cardY = cy - cardSize / 2 - S(25);
  const cardRadius = S(110);
  ...
  ctx.shadowOffsetX = S(20);
  ctx.shadowOffsetY = S(60);
  ...
  const pearlGrad = ctx.createLinearGradient(
    cardX,
    cardY,
    cardX + cardSize,
    cardY + cardSize,
  ...
  pearlGrad.addColorStop(1, rgba(softer, 0.2));
  ...
  const rayOriginX = cx - S(25);
  const rayOriginY = cy + S(35);
  ...
  for (const angle of rayAngles) {
    ...
    const rayLength = S(720);
    const rayWidth = S(38 + Math.abs(angle) * 0.7);

    const rayGrad = ctx.createLinearGradient(0, 0, 0, -rayLength);
    rayGrad.addColorStop(0, rgba(lighter, 0.2));
    rayGrad.addColorStop(0.3, rgba(lighter, 0.08));
    rayGrad.addColorStop(1, "rgba(255,255,255,0)");

    ctx.fillStyle = rayGrad;
    ctx.filter = `blur(${S(9)}px)`;
    ctx.beginPath();
  ...
  ctx.font = `bold ${S(490)}px "Georgia", "Times New Roman", "Palatino Linotype", serif`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";

  const letterX = cx;
  const letterY = cy + S(28);
  ...
  ctx.save();
  ctx.globalCompositeOperation = "screen";
  ctx.filter = `blur(${S(1.2)}px)`;
  ctx.fillStyle = rgba(lighter, 0.52);
  ctx.fillText(initial, letterX - S(4), letterY - S(5));
  ctx.restore();
  ...
  const orbAura = ctx.createRadialGradient(
    orbX,
    orbY,
    orbR * 0.5,
    orbX,
  ...
  const sparklesInside = [
    { x: cardX + cardSize * 0.11, y: cardY + cardSize * 0.21, r: S(14) },
    { x: cardX + cardSize * 0.87, y: cardY + cardSize * 0.66, r: S(10) },
    { x: cardX + cardSize * 0.74, y: cardY + cardSize * 0.83, r: S(8) },
    { x: cardX + cardSize * 0.17, y: cardY + cardSize * 0.79, r: S(7) },
    { x: cardX + cardSize * 0.56, y: cardY + cardSize * 0.09, r: S(6) },
  ];
  ...
  const sparklesOutside = [
    { x: cardX - S(38), y: cardY + cardSize * 0.44, r: S(11) },
    { x: cardX + cardSize + S(32), y: cardY + cardSize * 0.28, r: S(9) },
    { x: cardX + cardSize * 0.28, y: cardY + cardSize + S(28), r: S(7) },
    { x: cardX + cardSize * 0.72, y: cardY + cardSize + S(20), r: S(6) },
  ];
  ...
  drawRoundedSquare(ctx, cardX, cardY, cardSize, cardRadius);
  ctx.clip();
  const innerVig = ctx.createRadialGradient(
    cx,
    cy,
    cardSize * 0.22,
    cx,
