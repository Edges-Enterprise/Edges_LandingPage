...
async function detectCountryFromIP(ip: string): Promise<string> {
  ...
  try {
    ...
    if (data.status === "success") {
      ...
      // Only cache if it's a supported country, otherwise default to ng
      const finalCode = SUPPORTED_COUNTRIES.includes(
        countryCode as SupportedCountry,
      )
        ? countryCode
        : "ng";

      // Cache the result
      ipCache.set(ip, { country: finalCode, timestamp: Date.now() });
...
// Helper to check if country is supported
...
export async function proxy(request: NextRequest) {
  ...
  if (shouldDetectCountry) {
    ...
    if (savedCountry && isSupportedCountry(savedCountry)) {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        if (testCountry && isSupportedCountry(testCountry)) {
  ...
  const countryMatch = pathname.match(
    new RegExp(`^\\/(${SUPPORTED_COUNTRIES.join("|")})(\\/|$)`),
  );
  if (countryMatch && isSupportedCountry(countryMatch[1])) {
  ...
  if (isDashboard) {
    ...
    try {
      ...
      // ✅ If status is pending, show pending page
