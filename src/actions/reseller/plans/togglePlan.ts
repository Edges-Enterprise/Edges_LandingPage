...
export async function togglePlan(planId: string): Promise<{
  ...
  try {
    ...
    if (existingConfig) {
      ...
      // No config exists yet. The default resting state for a plan with
      // no config is DISABLED (see getPlans.ts), so the first time a
      // reseller hits this toggle it means "turn it on".
      newEnabled = true;
