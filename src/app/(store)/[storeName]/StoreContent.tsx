...
export function StoreContent({
  ...
  const [customerData, setCustomerData] = useState<{
    wallet: { id: string; balance: number; total_spent: number };
    virtualAccounts: any[];
  }>({ wallet: { id: "", balance: 0, total_spent: 0 }, virtualAccounts: [] });
  ...
  useEffect(() => {
    async function fetchCustomerData() {
      ...
      if (isOwner) {
        // For store owner: get their virtual accounts and wallet
        ...
        setCustomerData({
          wallet: wallet || { id: "", balance: 0, total_spent: 0 },
          virtualAccounts: accounts || [],
        });
      } else {
        // For customer: get their wallet and virtual accounts
        const data = await getCustomerWalletWithAccounts(
          user.id,
          resellerData.id,
        );
        setCustomerData({
  ...
  const handlePlusClick = () => {
    ...
    if (customerData.virtualAccounts.length > 0) {
  ...
  return (
    ...
    >
      ...
      {showFundingModal && (
        ...
        >
          ...
          >
            ...
            {customerData.virtualAccounts.map((account: any) => (
              <div
                key={account.id}
                style={{
                  background: "#F9FAFB",
      ...
      {/* ─── Wallet Balance Strip (replaces hero strip) ─── */}
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  {showBalance
                    ? formatNaira(customerData.wallet.balance)
                    : "••••••"}
                </span>
                <button
...
function PlanGrid({
  ...
  const gridStyles: React.CSSProperties = {
    display: "grid",
    gap: "0.5rem",
    gridTemplateColumns: "repeat(3, 1fr)", // Default mobile: 3 columns
  };
