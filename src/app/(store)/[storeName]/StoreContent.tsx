...
export function StoreContent({
  ...
  //    console.log("Reseller ID:", resellerData?.id);
  ...
  const handleCreateVirtualAccount = async () => {
    ...
    // ✅ FIX: Get the customer's UUID from reseller_customers table
    // const { data: customer } = await supabase
    //   .from("reseller_customers")
    //   .select("id")
    //   .eq("reseller_id", resellerData.id)
    ...
    const result = await createCustomerVirtualAccount(
      resellerData.id,
      // customer.id, // ← Use the customer's UUID, not user.id
      storeName,
    );
...
//   const [resellerDataState, setResellerDataState] = useState<{
//     id: string;
//   } | null>(null);
//   const [userState, setUserState] = useState<{ id: string } | null>(null);
