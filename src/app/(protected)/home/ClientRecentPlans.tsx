...
import { DEFAULT_PROVIDER_IMAGE } from "@/constants/helper";
...
export default function ClientRecentPlans({
  ...
  const handleSwipePurchase = async (plan: PopularPlan) => {
    ...
    try {
      // Navigate to the data provider page with the network code
      const networkCode = plan.provider.toLowerCase();
      router.push(`/data/${networkCode}`);
    } catch (error) {
