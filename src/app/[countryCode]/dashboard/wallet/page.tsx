...
export default async function WalletPage({ params }: WalletPageProps) {
  ...
  if (!walletData) {
    // We don't know the reseller's real country yet, so fall back to
    // whatever the URL said just to render a translated error message.
    const fallbackConfig = getCountryConfig(urlCountryCode);
    const fallbackTranslations = await getTranslations(
      fallbackConfig.language.code || "en",
    );
    return (
      <div
        style={{
  ...
  // ── Source of truth is the reseller's own record ────────────────
  // The URL's countryCode is untrusted routing info — it can be stale
  // (an old tab, a bookmarked link, a leftover redirect) and must never
  // drive gateway/currency selection or country-specific copy. If it
  // doesn't match the reseller's actual country, send them to the URL
  // that does. Everything below this point uses the verified value.
  const realCountryCode = walletData.application.country_code;

  if (realCountryCode && realCountryCode !== urlCountryCode) {
    redirect(`/${realCountryCode}/dashboard/wallet`);
  }

  const config = getCountryConfig(realCountryCode);
