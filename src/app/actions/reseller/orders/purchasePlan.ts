...
export async function purchasePlan(
  ...
  // 8-10. Get customer and customer wallet (ONLY for customers, NOT resellers)
  ...
  // ============================================================
  // 16. PROCESS DEDUCTIONS - DIFFERENT FOR RESELLER VS CUSTOMER
  // ============================================================

  if (isOwner) {
    // ✅ RESELLER SELF-PURCHASE: Just deduct cost price
    console.log("[Purchase] Reseller self-purchase - deducting cost only:", {
      resellerId: reseller.id,
    ...
    if (deductionError || !deductionResult?.success) {
      console.error("[Purchase] CRITICAL: Reseller cost deduction failed!", {
        resellerId: reseller.id,
        amount: plan.amount,
        requestId,
      ...
      await supabase.from("reseller_transactions").insert({
        ...
        metadata: {
          ...
          error: "Deduction failed (reseller self-purchase)",
    ...
    console.log(
      "[Purchase] Reseller cost deduction successful:",
      deductionResult,
    );
  } else {
    // ✅ CUSTOMER PURCHASE: Use process_purchase_deductions
    console.log("[Purchase] Processing customer deductions via RPC:", {
    ...
    if (deductionError || !deductionResult?.success) {
      console.error("[Purchase] CRITICAL: Customer deductions failed!", {
        resellerId: reseller.id,
    ...
    console.log("[Purchase] Customer deductions successful:", deductionResult);
  }

  // 17. Create completed order
  const { data: orderId, error: orderError } = await supabase.rpc(
    "create_purchase_order",
    {
      ...
      p_customer_email: user.email,
      p_plan_id: basePlan.id,
      p_amount: isOwner ? plan.amount : finalPrice, // Cost price for reseller, selling price for customer
      p_profit: isOwner ? 0 : profit, // No profit for reseller self-purchase
      p_status: "completed",
    },
  );
  ...
  // 18. RECORD CUSTOMER TRANSACTION (Only for customer purchases)
  if (!isOwner && customerWallet) {
  ...
  // 19. Record reseller transaction
  await supabase.from("reseller_transactions").insert({
    reseller_id: reseller.id,
    amount: isOwner ? plan.amount : finalPrice,
    type: "purchase",
    status: "completed",
    ...
    metadata: {
      ...
      lizzy_ref: requestId,
      reseller_balance_before: resellerBalance,
      is_reseller_self_purchase: isOwner,
    },
  });
